...
def parse_vacancy(vacancy):
    ...
    url_vacancy = vacancy.find("a", class_="base-card__full-link absolute top-0 right-0 bottom-0 left-0 p-0 z-[2]").get("href")
    ...
    div_element = soup_detail.select_one(".show-more-less-html__markup.show-more-less-html__markup--clamp-after-5")
...
def parse_all_page(start):
    while start < 1000:
        url = "https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=&location=&geoId=101728296&trk=guest_homepage-basic_jobs-search-bar_search-submit&start=" + str(start)
        print(f"Обрабатывается страница {url}")
        r = requests.get(url)
        print(f"Status Code: {r.status_code}")
        print(f"Status Message: {r.reason}")
        soup = BeautifulSoup(r.text, "lxml")
        all_vacancy_on_page = soup.findAll("div", class_="base-card relative w-full hover:no-underline focus:no-underline base-card--link base-search-card base-search-card--link job-search-card")
    ...
    while start < 1000:
        url = "https://www.linkedin.com/jobs-guest/jobs/api/seeMoreJobPostings/search?keywords=&location=&geoId=101705918&trk=public_jobs_jobs-search-bar_search-submit&start=" + str(start)
        print(f"Обрабатывается страница {url}")
        r = requests.get(url)
        print(f"Status Code: {r.status_code}")
        print(f"Status Message: {r.reason}")
